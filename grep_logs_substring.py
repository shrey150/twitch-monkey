...
def grep_word(index_word, logs_file):
    ...
    try:
        ...
        if result.returncode == 0:  # if grep found a match
            ...
            # Ignore any chatter with the word "bot" in their username
            if 'bot' in chatter.lower():
            ...
            # Replace only the first instance of the word
            highlighted_text = re.sub(f'({word})', r'(\1)', chatter_text, count=1, flags=re.IGNORECASE)
            ...
            return index, f"{formatted_timestamp} {chatter}: {highlighted_text}"
        else:
            # If no match was found
            print(f"No match found for {word}")
            # raise ValueError(f'No match found for {word}')
